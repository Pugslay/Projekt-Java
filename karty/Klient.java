...
//Klasa odpowiedzialna za odczytywanie i interpretację graficzną komunikatów oraz rozgrywkę
public class Klient {
            ...
            case "usun" ->{
                ...
                switch (pozycja){
                    ...
                    case "panel4" -> {
                        panel4.removeAll();
                        panel4.setBorder(BorderFactory.createLineBorder(Color.RED, 3, true));
                        CP.InsertGraph(iconlabel[0],panel4, Integer.toString(insertKart.szukajATA(ID)), HP_J, Integer.toString(insertKart.szukajPAN(ID)), Integer.toString(insertKart.szukajKOSZT(ID)), InsertKart.szukajNazwa(ID), insertKart.szukajEfekty(ID));
                        panel4.repaint();
                    }
                }
